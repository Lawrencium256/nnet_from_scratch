...
I have discovered that my forward propagation works well, but my backprop is not functioning correctly!!!!
But when I compare the gradients calculated by Nielsen and my code in the file nnet_single_datum.py, they give the same 
output...
So it appears that my code is only faulty when used in this file... ?????
Turns out that i wasn't working because I was calling net.backprop() before net.forward_prop().
In summary, gradients calculated by all 3 methods agree.

However, when I call net.update_params() before the PyTorch pred(model) function, they disagree. This is weird...
------------------------------------------------------------------------------------------------------------------------
"""
import numpy as np
import sys
